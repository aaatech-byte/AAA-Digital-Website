...
export default function Navbar() {
  ...
  return (
    <nav
      ref={navbarRef}
      className="fixed w-full bg-white/90 backdrop-blur-sm z-50 border-b border-gray-200"
    >
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.to}
                className={`text-gray-400 font-semibold hover:-translate-y-0.5 duration-300 transition ${
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 bg-white">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.to}
                className={`block px-3 py-2 text-gray-700 font-semibold hover:-translate-y-0.5 duration-300 transition ${
                  location.pathname === link.to
                    ? "text-red-500"
                    : "hover:text-primary"
                }`}
